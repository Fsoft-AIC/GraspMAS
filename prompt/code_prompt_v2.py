context = '''
You are an expert software programmer. Use a Chain-of-Thought approach to break down the problem, create pseudocode, and then write the code in Python language. Ensure that your code is efficient, readable, and well-commented. Return the requested information from the function you create.
Setting:
...
Consider the following guidelines:
    Objective:
    - Your primary goal is to identify a feasible grasp pose for a given object using RGB inputs. You must ensure that the solution adheres to physical common sense and avoids harming people or objects.
    - The function should return the grasp pose of the object (ONLY ONE) specified in the query.
...
Plan:
Step 1: Locate all instances of "clock" in the image. Use the find function with the object name "clock."
Step 2: Check the color property of each detected clock. Use the verify_property function to check if the clock is black.
...
Plan: 
Step 1: Check if "foo" exists in the image. Use the exists function with the object name "foo."
Step 2: Check if "garply bar" exists in the image. Use the exists function with the object name "garply bar."
...
Plan:
Step 1: Find the carrot in the image. Use the find function with the object name "carrot."
Step 2: Detect the grasp pose for the first detected carrot. Use the grasp_detection function with the first carrot patch.
A: ```
def execute_command(image):
    image_patch = ImagePatch(image)
...
### Example 4
Q: Write a python code to solve the following coding problem that obeys the constraints. The output code needs to {FEW_SHOT_QUESTION_GUIDE}. Please wrap your code answer using ```: 
Query: Give me the blue and red bottle
Plan:
Step 1: Find all patches containing bottles in the image. Use the find method with the object name "bottle."
Step 2: Iterate through each detected bottle patch. 
Step 3: Verify if the bottle is both blue and red. Use the verify_property method with the object "bottle" and property both "blue" and "red".
Step 4: Perform grasp pose detection for the blue bottle. Use the grasp_detection method with the blue bottle patch.
...
def execute_command(image):
    ...
    for bottle_patch in bottle_patches:
        if bottle_patch.verify_property("bottle", "blue and black"):
...
Plan:
Step 1: Find all patches containing chocolate bars in the image. Use the find method with the object name "chocolate bar."
Step 2: Sort the chocolate bar patches based on their horizontal position. Use the sort method with the horizontal_center attribute, larger values are closer to the right.
Step 3: The second chocolate bar from the left will be the second element in the sorted list.
Step 4: Return the grasp pose for the second chocolate bar from the left. Use the grasp_detection method with the second chocolate bar patch.
A: ```
def execute_command(image):
    image_patch = ImagePatch(image)
...
Plan:
Step 1: Find all objects in the image. Use the find method with the object name "object."
Step 2: Sort the objects based on their vertical position. Use the sort method with the vertical_center attribute, larger values are closer to the top.
Step 3: Select the object with the highest vertical center. This will be the last object in the sorted list.
Step 4: Perform grasp pose detection for the selected object. Use the grasp_detection method with the selected object patch.
...
Plan: 
Step 1: Detect all knives in the image. Use the find method of ImagePatch to locate all objects identified as "knife" in the image. Take the first detected knife patch.
Step 2: To handover to the user safely, locate the "blade" part of the first detected knife. Use the find_part method on the first detected knife patch to locate the "blade" part of the knife.
Step 3: Calculate the grasp pose for the knife blade. Use the grasp_detection method of ImagePatch to calculate the grasp pose for the detected knife blade.
A: ```
def execute_command(image):
    image_patch = ImagePatch(image)
    knife_patches = image_patch.find("knife")
...
### Example 8
Q: Write a python code to solve the following coding problem that obeys the constraints. The output code needs to {FEW_SHOT_QUESTION_GUIDE}. Please wrap your code answer using ```: 
Query: please hand me the coffee cup. Since the coffee cup is very hot, you should grasp it by the body, allowing me to safely grasp the handle without risking a burn.
Plan: 
Step 1: Detect all cups in the image. Use the find method of ImagePatch to locate all objects identified as "cup" in the image. This will return a list of patches corresponding to detected cups.
Step 2: Identify the body of the first detected cup. Use the find_part method on the first detected cup patch to locate the "body" part of the cup. This ensures the assistant can grasp the cup by its body, leaving the handle accessible.
Step 3: Perform grasp pose detection for the cup body. Use the grasp_detection method of ImagePatch to calculate the grasp pose for the detected cup body.
...
### Example 9
Q: Write a python code to solve the following coding problem that obeys the constraints. The output code needs to {FEW_SHOT_QUESTION_GUIDE}. Please wrap your code answer using ```: 
Query: Grasp the plant. To ensure safe handling, grasp the pot of the plant.
Plan: 
Step 1: Detect all plants in the image. Use the find method of ImagePatch to locate all objects identified as "plant" in the image. This will return a list of patches corresponding to detected plants.
Step 2: Identify the pot of the first detected plant. Use the find_part method on the first detected plant patch to locate the "pot" part of the plant. Grasping the pot ensures safe handling of the plant.
Step 3: Perform grasp pose detection for the plant pot. Use the grasp_detection method of ImagePatch to calculate the grasp pose for the detected plant pot.
A: ```
def execute_command(image):
    image_patch = ImagePatch(image)
    plant_patches = image_patch.find("plant")
    plant_pot_patche = plant_patches[0].find_part("plant", "pot")
    grasp_pose = image_patch.grasp_detection(plant_pot_patche)
    return grasp_pose
    ```
'''

FEW_SHOT_QUESTION_GUIDE='''use basic math, logical operator Python and the ImagePatch class to execute and achieve the task in the coding problem'''
examples_context = examples.replace("{FEW_SHOT_QUESTION_GUIDE}", FEW_SHOT_QUESTION_GUIDE)
